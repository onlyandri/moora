...
mysqli_query($conn,"DELETE FROM penilaian where kode_pelamar='$kode_pelamar'");

//memasukan data baru
foreach ($kode_kriteria as $key => $value) {
	$query = mysqli_query($conn,"INSERT INTO `penilaian` (`tanggal_penilaian`,`kode_pelamar`, `kode_kriteria`, `nilai`) VALUES (NOW(),'$kode_pelamar', '$value', '".$_POST['jawaban_'.$value]."')");
}
